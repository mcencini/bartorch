...
t2_table = np.array([float(row["T2 (ms)"]) for row in rows], dtype=np.float32)
...
occupancy = memberships.sum(0)
support = occupancy > 0.5

amplitude = (memberships * torch.as_tensor(proton_density[list(TISSUES)])[:, None, None]).sum(0)
t2 = (memberships * torch.as_tensor(t2_table[list(TISSUES)])[:, None, None]).sum(0)
t2 = t2 / occupancy.clamp(min=1e-6)
t2 = torch.where(support, t2.clamp(20.0, 400.0), torch.tensor(50.0))
amplitude = torch.where(support, amplitude, torch.zeros(()))
# sphinx_gallery_end_ignore
...
for axis, values, title in (
    ...
    handle = axis.imshow(
        torch.where(support, values, torch.zeros(())).detach().cpu().numpy(),
        cmap="viridis",
        vmin=0.0,
        vmax=200.0,
    )
    axis.set_title(title, fontsize=10)
figure.colorbar(handle, ax=axes[0, 2], label="$T_2$ [ms]", fraction=0.046)
