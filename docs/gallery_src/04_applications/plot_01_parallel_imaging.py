...
coil_images = torch.fft.fftshift(torch.fft.ifft2(
    torch.fft.ifftshift(full, dim=(-2, -1)), norm="ortho"
), dim=(-2, -1)).reshape(4, n, n)
...
for ax, value, title in zip(
    axes, [reference, result, (result - reference).abs()],
