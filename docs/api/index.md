...
| {doc}`learning` | `bartorch.learning`: a real-valued denoiser over complex images, and an iteration as a network |
...
| {doc}`interop` | `bartorch.interop`: an operator as a deepinv physics, for its samplers and its physics-coupled losses |
