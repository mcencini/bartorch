"""A BART command as a Python function, built from the catalogue.

Every command BART builds is reachable, and the ones nobody has written a
wrapper for are built from their catalogue entry: a real signature, real
defaults, BART's own help as the docstring, and each option routed to the flag
BART actually spells it with.

These are not the surface anyone should prefer.  A derived wrapper knows only
what BART's option table says, so a trajectory is an array because BART calls
it a file, but nothing tells it that the trajectory and the basis belong to the
same transform, or that two of the options are the same choice.  That is what
the curated modules beside this one are for, and :mod:`bartorch.tools` prefers
...
__all__ = ["build", "curated", "derived_names", "signature_for"]


def curated(command: str):
    """Mark a hand-written wrapper as the one that covers *command*.

    The coverage audit reads the mark rather than a second list, so a wrapper
    cannot be written and left out of the count.
    """

    def mark(function):
        function.bart_command = command
...
def build(name: str):
    """The derived wrapper for one BART command.

    The function it returns carries the signature and the docstring, so
    ``help()`` and an editor's completion show what the command takes without
    any of it having been written out.
    """
    ...
    call.__module__ = "bartorch.tools"
...
def derived_names() -> tuple[str, ...]:
    """Every command the catalogue knows, in BART's own order."""
